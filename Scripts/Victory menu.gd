...
func _process(_delta):
	if Network.is_networking:
		...
		$CanvasLayer/VBoxContainer2/energys.text = "Energys Balls:" + str(Network.energys[0] + Network.energys[1] + Network.energys[2] + Network.energys[3] )
		$CanvasLayer/VBoxContainer2/score.text = "Score:" + str(Network.score[0] + Network.score[1] + Network.score[2] + Network.score[3])
	else:
		$CanvasLayer/Label.text = "You Completed Level: \n level " + str(Globals.level_int - 1) + "!!!"
